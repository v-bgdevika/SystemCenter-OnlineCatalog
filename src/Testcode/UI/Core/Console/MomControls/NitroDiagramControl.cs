...
//  </history>
//-------------------------------------------------------------------
namespace Mom.Test.UI.Core.Console.MomControls.Nitro
{
    using System;
    ...
    //using SystemCenter.Test.UI.Core.ConsoleFramework;
    //using SystemCenter.Test.UI.Core.ConsoleFramework.Utilities;
    ...
    public class DiagramControl
    {
        ...
        public DiagramControl(Window window)
        {
            ...
            {
                Core.Common.Utilities.LogMessage("No Vertical scroll bar was found");
            ...
            {
                Core.Common.Utilities.LogMessage("No Horizontal scroll bar was found");
        ...
        public VerticalScrollBar VerticalScrollBar
        {
            get
            {
                ...
                {
                    Core.Common.Utilities.LogMessage("Attempting to access vertical scrollbar, but it is currently null.  Trying again to get a reference");
        ...
        public HorizontalScrollBar HorizontalScrollBar
        {
            get
            {
                ...
                {
                    Core.Common.Utilities.LogMessage("Attempting to access horizontal scrollbar, but it is currently null.  Trying again to get a reference");
        ...
        {
            Core.Common.Utilities.LogMessage(String.Format("FindNode Id [{0}]", nodeId));
        ...
        {
            Core.Common.Utilities.LogMessage(String.Format("FindEdge Id [{0}]", edgeId));
            ...
            if (listViewItem != null)
            {
                ...
                {
                    Core.Common.Utilities.LogMessage(String.Format("Edge with Id [{0}] found!", edgeId));
                ...
                while (((listViewItem = walker.GetNextSibling(listViewItem)) != null))
                {
                    if (listViewItem.Current.AutomationId.CompareTo(edgeId) == 0)
                    {
                        Core.Common.Utilities.LogMessage("Edge with Id [{0}] found: " + edgeId);
                        return new EdgeControl(listViewItem);
                    }
                }
            }
            else
            {
                ...
                Core.Common.Utilities.LogMessage("No edge found matching id, in fact no edges at all found");
                return null;
            }
        ...
        private EdgeControl FindEdge(AutomationElement treeViewAutomationElement, string edgeId)
        {
            ...
            else
            {
                ...
                Core.Common.Utilities.LogMessage("No nodes found in TreeView");
                return null;
            }

            Core.Common.Utilities.LogMessage("No matching node found in TreeView");
            return null;
        }
        ...
        private NodeControl FindNode(AutomationElement treeView, string nodeId)
        {
            ...
            if (treeViewItem != null)
            {
                ...
                {
                    Core.Common.Utilities.LogMessage(String.Format("Node with Id [{0}] found!", nodeId));
        ...
        public class NodeControl
        {
            ...
            public bool Boxed
            {
                get
                {
                    return this.GetBoxUnboxItemPattern().Current.Value.ToLower().Contains("box=true");
                }
            }
            ...
            {
                Core.Common.Utilities.LogMessage(String.Format("Ensuring node {0} is visible", this.node.Current.AutomationId));
            ...
            public void Select()
            {
                Core.Common.Utilities.LogMessage(String.Format("Selecting node {0}", this.node.Current.AutomationId));
            ...
            public void SendKeys(string keys)
            {
                Core.Common.Utilities.LogMessage(String.Format("Sending keys {0} to node {1}", keys, this.node.Current.AutomationId));
                this.SendKeys(keys);
            }
            ...
            {
                Core.Common.Utilities.LogMessage(String.Format("UnSelecting node {0}", this.node.Current.AutomationId));
            ...
            public void Expand()
            {
                Core.Common.Utilities.LogMessage(String.Format("Expanding node {0}", this.node.Current.AutomationId));
            ...
            {
                Core.Common.Utilities.LogMessage(String.Format("Collapsing node {0}", this.node.Current.AutomationId));
            ...
            {
                Core.Common.Utilities.LogMessage(String.Format("Bounding rectangle {0} for node {1}", this.node.Current.BoundingRectangle, this.node.Current.AutomationId));
            ...
            public void Box()
            {
                Core.Common.Utilities.LogMessage(String.Format("Boxing node {0}", this.node.Current.AutomationId));
            ...
            public void UnBox()
            {
                Core.Common.Utilities.LogMessage(String.Format("UnBoxing node {0}", this.node.Current.AutomationId));
                ValuePattern valuePattern = this.GetBoxUnboxItemPattern();
        ...
        public class EdgeControl
        {
            ...
            public void Select()
            {
                Core.Common.Utilities.LogMessage(String.Format("Selecting edge {0}", this.edge.Current.AutomationId));
                this.GetSelectionItemPattern().Select();
            }
            ...
            public void SendKeys(string keys)
            {
                Core.Common.Utilities.LogMessage(String.Format("Sending keys {0} to edge {1}", keys, this.edge.Current.AutomationId));
                this.SendKeys(keys);
            }
            ...
            public void EnsureVisible()
            {
                Core.Common.Utilities.LogMessage(String.Format("Ensure edge is visible {0}", this.edge.Current.AutomationId));
                this.GetScrollItemPattern().ScrollIntoView();
            }
            ...
            public Rect GetBoundingRectangle()
            {
                Core.Common.Utilities.LogMessage(String.Format("Bounding rectangle {0} for node {1}", this.edge.Current.BoundingRectangle, this.edge.Current.AutomationId));
            ...
            public void UnSelect()
            {
                Core.Common.Utilities.LogMessage(String.Format("Removing edge {0} from selection", this.edge.Current.AutomationId));
                this.GetSelectionItemPattern().RemoveFromSelection();
            }
            ...
            {
                Core.Common.Utilities.LogMessage("Getting source node id for edge " + this.edge.Current.AutomationId);
            ...
            {
                Core.Common.Utilities.LogMessage("Getting sink node id for edge " + this.edge.Current.AutomationId);
