   IF NOT EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'dbo.fn_VersionCompare') and xtype in (N'FN', N'IF', N'TF'))
    EXECUTE('CREATE FUNCTION dbo.fn_VersionCompare() RETURNS tinyint AS BEGIN RETURN NULL END')
  GO

  ALTER FUNCTION dbo.fn_VersionCompare
  (
